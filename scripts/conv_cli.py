...
def conv_root(args: argparse.Namespace | None = None) -> Path:
    ...
    root = explicit or os.environ.get("BRAIN_CONV")
    if root:
        return Path(root).expanduser().resolve()
    return (repo_root_from_script() / "conv").resolve()
...
def normalize_meta(raw: dict[str, Any], *, existing: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not cid:
        cid = make_unique_id(topic, conv_root(argparse.Namespace(conv_root=None)))
...
def upsert(root: Path, raw: dict[str, Any], *, status_override: str | None = None) -> dict[str, Any]:
    ...
    meta = normalize_meta(raw, existing=existing_meta)
    body = build_body(raw)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="conv conversation store helper")
    parser.add_argument("--conv-root", help="absolute path to conv root; defaults to BRAIN_CONV or repo conv/")
    sub = parser.add_subparsers(dest="cmd", required=True)

    p = sub.add_parser("init", help="create conv layout and rebuild index")
    p.set_defaults(func=cmd_init)

    p = sub.add_parser("rebuild-index", help="rebuild index.jsonl from conv/log/*.md")
    p.set_defaults(func=cmd_rebuild_index)

    p = sub.add_parser("regen-refs", help="reconcile bidirectional refs and rebuild index")
    p.set_defaults(func=cmd_regen_refs)

    p = sub.add_parser("upsert", help="create or replace a conversation markdown file")
    ...
    p = sub.add_parser("set-status", help="set a conversation status")
    p.add_argument("id")
    p.add_argument("status", choices=sorted(STATUSES))
    p.set_defaults(func=cmd_set_status)

    p = sub.add_parser("list", help="list conversations from index.jsonl")
    p.add_argument("--status", choices=sorted(STATUSES))
    p.add_argument("--limit", type=int, default=10)
    p.add_argument("--json", action="store_true")
    p.set_defaults(func=cmd_list)

    p = sub.add_parser("search", help="tiered search: filename, index fields, body fallback")
    p.add_argument("query")
    p.add_argument("--limit", type=int, default=10)
    p.set_defaults(func=cmd_search)

    p = sub.add_parser("show", help="show one conversation by id or search query")
    p.add_argument("target")
    p.add_argument("--markdown", action="store_true")
    p.set_defaults(func=cmd_show)

    p = sub.add_parser("doctor", help="validate layout, parseability, and optional tools")
